...
// Mock config if not available, or assume interface
// Since this is a lib, strictly it should receive config or assume env.
// For now, I'll replicate the logic using process.env directly to decouple from backend appConfig
...
function getCallerInfo(): { file: string; line: string; full: string } {
  try {
    ...
    for (let i = 1; i < stack.length; i++) {
      ...
      if (
        filename.includes("logger.ts") || // Self
        filename.includes("index.ts") || // Self (if index)
...
function logAtLevel(
  ...
  if (thisLevelValue < currentLevelValue) return;
...
export const logger = {
  debug: (msg: string, meta?: Record<string, unknown>) =>
    logAtLevel("debug", msg, meta),
  info: (msg: string, meta?: Record<string, unknown>) =>
    logAtLevel("info", msg, meta),
  warn: (msg: string, meta?: Record<string, unknown>) =>
    logAtLevel("warn", msg, meta),
  error: (msg: string, meta?: Record<string, unknown>) =>
    logAtLevel("error", msg, meta),
};
