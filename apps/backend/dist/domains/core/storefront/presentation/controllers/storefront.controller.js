...
const setupStorefrontUseCase = new SetupStorefrontUseCase(storefrontRepo, organizationRepo);
export class StorefrontController {
    /**
     * getPublicStorefront
     * Fetches storefront data by slug for the public website.
     */
    static async getPublicStorefront(req, res) {
     ...
     */
    static async getAdminStorefront(req, res) {
        const organizationId = req.user?.organization; // Explicit cast to avoid lint error
        if (!organizationId) {
            return res
                .status(401)
                .json({ message: "Unauthorized: No organization context" });
        }
        let storefront = await getStorefrontUseCase.executeByOrganization(organizationId);
        // Lazy initialization if not exists
        if (!storefront) {
            const organization = await organizationRepo.findById(organizationId);
            if (organization) {
                storefront = await setupStorefrontUseCase.execute(organizationId, organization.slug);
            }
        }
        if (!storefront) {
            return res.status(404).json({ message: "Storefront not found" });
     ...
     */
    static async updateStorefront(req, res) {
        ...
        const updated = await updateStorefrontUseCase.execute(organizationId, req.body);
