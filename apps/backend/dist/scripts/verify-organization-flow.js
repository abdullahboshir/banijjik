...
async function verify() {
    ...
    const createUserUseCase = new CreateUserUseCase(userRepo, passwordService, uniqueEmailPolicy, passwordStrengthPolicy);
    const createMembershipUseCase = new CreateOrganizationMembershipUseCase(membershipRepo);
    const iamAdapter = new IamServiceAdapter(createUserUseCase, createMembershipUseCase, jwtService);
    const useCase = new CreateOrganizationUseCase(orgRepo, iamAdapter, mailService);
    const mockDto = {
...
verify();
//# sourceMappingURL=verify-organization-flow.js.map