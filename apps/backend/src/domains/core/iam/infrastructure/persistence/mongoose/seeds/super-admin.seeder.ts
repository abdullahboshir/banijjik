...
export async function seedSuperAdmin() {
  ...
  if (!superAdminRole) {
    throw new Error(
      "CRITICAL: SUPER_ADMIN role not found. Run platform seeder first.",
    );
  }
  ...
  const existingLegacyUser = await coreCollection.findOne({
    _id: SUPER_ADMIN_ID as any,
  });

  if (existingLegacyUser) {
    console.log(
      "   ⚠️  Found legacy string ID for Super Admin. Migrating to ObjectId...",
    );
    await coreCollection.deleteOne({ _id: SUPER_ADMIN_ID as any });
  }
